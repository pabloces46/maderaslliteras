...
$fechabackup = $resultado->fetch_assoc();

$fecha = date('d')."-".date('m')."-".date('Y');

//echo $fechabackup['FECHA'] ." - ". $fecha;


if($fechabackup['FECHA'] != $fecha){
	/**
	...
	class Backup_Database {
		...
		public function __construct($host, $username, $passwd, $dbName, $charset = 'utf8') {
			...
			$this->backupFile      = $this->dbName.'-backup-'.date("d-m-Y").'.sql';
	...
	$mysqli = ConectarDB();
	if (!$mysqli->query("UPDATE backup SET FECHA = '$fecha' WHERE FECHA = '$fechabackup[FECHA]' ")) {
